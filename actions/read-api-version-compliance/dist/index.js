...
function checkYamlFile(filePath, repoPath) {
    const issues = [];
...
function checkFeatureFile(filePath, repoPath) {
    const issues = [];
    const relativePath = path.relative(repoPath, filePath);
    let content;
    try {
...
async function run() {
    try {
        ...
        if (fs.existsSync(apiDefPath)) {
            const yamlFiles = await (0, glob_1.glob)('**/*.{yaml,yml}', { cwd: apiDefPath, nodir: true });
        ...
        if (fs.existsSync(testDefPath)) {
            const featureFiles = await (0, glob_1.glob)('**/*.feature', { cwd: testDefPath, nodir: true });
