...
function determineReleaseState(latestPublicRelease, newestPreRelease) {
...
function getPrereleaseType(releaseType) {
    if (!releaseType)
        return 'pre-release';
    if (releaseType.includes('alpha'))
        return 'alpha';
    if (releaseType.includes('beta'))
        return 'beta';
    if (releaseType.includes('rc'))
        return 'rc';
    return 'pre-release';
}
(async () => {
    try {
        ...
        const doc = yaml.load(fs.readFileSync(releasesFile, 'utf8'));
        if (!doc || !doc.releases) {
        ...
        const repoReleases = doc.releases.filter((r) => r.repository === repoName);
        // Find public release details (if exists)
        let publicReleaseData = null;
